  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      try {
        ...
        const result = await projectsAPI.importJson(projectData, token)
    ...
    onError: (error) => {
      setUploadResults(prev => [...prev, error])
    }
  })
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const file of acceptedFiles) {
      try {
        ...
        // Enhanced validation
        const validationErrors = validateProjectData(projectData, file.name)
        ...
        previews.push({
          ...
          shortDesc: projectData.shortDesc || 'No description provided',
          level: projectData.level || 'BEGINNER',
          guidance: projectData.guidance || 'FULLY_GUIDED',
